...
let delay = 400 - speedSlider.value;
...
function generateRandomArray() {
  ...
  for (let i = 0; i < n; i++) {
    ...
    bars[i].style.backgroundColor = '#F6F6F2';
...
function draw(coloredBars, colors) {
  ...
  for (let i = 0; i < n; i++) {
    bars[i].style.backgroundColor = "#697184";
...
async function SortedAnimation() {
  ...
  for (let i = 0; i < n; i++) {
    bars[i].style.backgroundColor = '#EFB7BA';
    await sleep(10);
  }
  await sleep(300);
  for (let i = 0; i < n; i++) {
    bars[i].style.backgroundColor = 'E5COC8';
    await sleep(10);
  }
}
  ...
  for (let i = 0; i < n - 1; i++) {
    ...
    for (let j = i + 1; j < n; j++) {
      ...
      if (!isPaused) {
        ...
        draw([i, j, minIndex], ['#F2F1EF', '#413F3D', '#B1A6A4']);
      } else {
        j--;
      }
      await sleep(delay);
      ...
      while (i < n1 || j < n2) {
        ...
        if (!isPaused) {
          if (j == n2 || (i < n1 && L[i] <= R[j])) {
            draw([k, ...barsIndices], ['#B1A6A4', ...barsColors]);
            i++;
          } else {
...
async function quickSort() {
  ...
  while (!s.isEmpty()) {
    ...
    for (let j = l; j <= h - 1; j++) {
      ...
      if (!isPaused) {
        draw([i, j, ...barsIndices], ['#B1A6A4', '#413F3D', ...barsColors]);
